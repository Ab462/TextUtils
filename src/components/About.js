...
export default function About() {
    ...
    const toggleStyle = async()=>{
        if (Style.color === '#042743') {
            setStyle({color:'white' , backgroundColor:'#042743' , transition:'1s',border:'1px solid white'})
            setBtnText('Enable Light Mode') 
        } else{
            setStyle({color:'#042743' , backgroundColor:'white' , transition:'1s'})
  ...
  return (
    <div className='container my-2' style={Style}>
        ...
        <div className="container my-3">
            <button onClick={toggleStyle} id="color-btn" type="button" className="btn btn-primary">{btnText}</button>
        </div>
    </div>
  )
}
